...
// TODO: implement the norm using sqrt and thrust::inner_product
         return std::sqrt( thrust::inner_product(X.begin(), X.end(), X.begin(), 0.0) );
}
...
// TODO:  make tuple using make_zip_iterator where T = (X,L,R,Y)
    thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple(X.begin(), L.begin(), R.begin(), Y.begin())),
                     thrust::make_zip_iterator(thrust::make_tuple(X.end(),   L.end(),   R.end(),   Y.end())),
